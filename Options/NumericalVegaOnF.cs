...
namespace TSLab.Script.Handlers.Options
{
    ...
    [HelperName("Numerical Vega ATM", Language = Constants.En)]
    [HelperName("Численная вега на деньгах", Language = Constants.Ru)]
    [InputsCount(4)]
    ...
    [Description("Численный расчет веги позиции в текущей точке БА (вычисляется одна точка)")]
    [HelperDescription("Numerical estimate of vega at-the-money (only one point is processed)", Constants.En)]
    public class NumericalVegaOnF : BaseContextHandler, IValuesHandlerWithNumber
    {
        private const string MsgId = "VEGA";
        /// <summary>
        ...
        private OptimProperty m_vega = new OptimProperty(0, false, double.MinValue, double.MaxValue, 1.0, 3);
        ...
        /// <summary>
        /// \~english Current vega (just to show it on ControlPane)
        /// \~russian Текущая вега всей позиции (для отображения в интерфейсе агента)
        /// </summary>
        [HelperName("Vega", Constants.En)]
        [HelperName("Вега", Constants.Ru)]
        [ReadOnly(true)]
        [Description("Текущая вега всей позиции (для отображения в интерфейсе агента)")]
        [HelperDescription("Current vega (just to show it on ControlPane)", Constants.En)]
        [HandlerParameter(true, NotOptimized = false, IsVisibleInBlock = true, Default = "0", IsCalculable = true)]
        public OptimProperty Vega
        {
            get { return m_vega; }
            set { m_vega = value; }
        }
