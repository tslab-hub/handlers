...
{
    /// <summary>
    /// \~english Open virtual position in option (stream handler)
    /// \~russian Создание виртуальной позиции в опционах (потоковый обработчик)
    /// </summary>
    [HandlerCategory(HandlerCategories.OptionsPositions)]
    [HelperName("Open Virtual Opt Pos", Language = Constants.En)]
    [HelperName("Открыть вирт. позицию в опционах", Language = Constants.Ru)]
    [InputsCount(1)]
    [Input(0, TemplateTypes.OPTION_SERIES, Name = Constants.OptionSeries)]
    [OutputsCount(1)]
    [OutputType(TemplateTypes.POSITION)]
    [Description("Создание виртуальной позиции в опционах (потоковый обработчик)")]
    [HelperDescription("Open virtual position in option (stream handler)", Constants.En)]
    public class OpenVirtualOptPosition : IContextUses, IStreamHandler 
    {
        ...
        public IPosition Execute(IOptionSeries optSer)
        {
            ...
            if (m_optionType == StrikeType.Put)
            {
                ISecurity sec = (from s in m_context.Runtime.Securities
                                 where (s.Symbol == pair.Put.Security.Symbol)
                                 select s).Single();
                int j = GetTodayOpeningBar(pair.Put.Security);
            ...
            {
                ISecurity sec = (from s in m_context.Runtime.Securities
                                 where (s.Symbol == pair.Call.Security.Symbol)
                                 select s).Single();
                int j = GetTodayOpeningBar(pair.Call.Security);
